...
export default function Map({ locationValue }: { locationValue: string }) {
  ...
  const country = getCountryByValue(locationValue)?.label;

  return (
    <MapContainer
      scrollWheelZoom={false}
      className="h-[50vh] rounded-lg relative z-0"
      center={latLang ?? [13.736717, 100.523186]}
      zoom={8}
    >
      <TileLayer
      ...
      <Marker position={latLang ?? [13.736717, 100.523186]} icon={ICON}>
        <Popup>{country ?? "Thailand"}</Popup>
      </Marker>
    </MapContainer>
  );
}
